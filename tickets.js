/* ===========================================================
   ServiceLink — Ticket List behaviour
   Domain values trace to RAD FR-2.2 (six states, four priorities).
   In-memory data; replace TICKETS with an API fetch in implementation.
   =========================================================== */
...
var AV_COLORS = ["#3d7eff","#7c5cff","#e07b39","#2b9d8f","#c4477b","#4a8fe0","#d6a02b"];
...
function slaPill(sla) {
  switch (sla) {
    case "new":      return '<span class="pill pill--new">New</span>';
    case "respond":  return '<span class="pill pill--respond">Response due</span>';
    case "overdue":  return '<span class="pill pill--overdue">Overdue</span>';
    case "resolved": return '<span class="pill pill--resolved">Resolved</span>';
    default:         return '<span class="pill pill--none">&mdash;</span>';
  }
}
...
function rowHTML(t) {
  ...
  return (
    '<tr data-id="' + t.id + '">' +
      '<td class="col-id">' + esc(t.id) + '</td>' +
      '<td class="col-subject"><span class="subject__title">' + esc(t.subject) + '</span></td>' +
      '<td class="col-requester"><span class="requester">' +
        '<span class="avatar" style="background:' + avatarColor(req) + '">' + initials(req) + '</span>' + req +
      '</span></td>' +
      '<td class="col-priority"><span class="priority pri-' + t.priority + '"><span class="pri-dot"></span>' + t.priority + '</span></td>' +
      '<td class="col-sla">' + slaPill(t.sla) + '</td>' +
      '<td class="col-status"><select class="status-select" aria-label="Status for ' + t.id + '">' + statusOptions(t.state) + '</select></td>' +
    '</tr>'
  );
}
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  /* theme toggle — in-memory; maps to User.theme_preference in production */
  var theme = "dark";
  ...
  toggle.addEventListener("click", function () { setTheme(theme === "dark" ? "light" : "dark"); });
  setTheme(theme);
